        ...
        <div class="documents-grid">
            ...
            <div class="document-card">
                <div class="document-header">
                    ...
                    <div class="document-actions">
                        <button class="doc-btn doc-btn-view" onclick="viewFullscreen('{{ Storage::url($application->id_document) }}', 'image')">
                            🔍 View
                        </button>
                        <a href="{{ Storage::url($application->id_document) }}" target="_blank" class="doc-btn doc-btn-download">
                            ⬇️ Download
                        </a>
                    </div>
                </div>
                <div class="file-preview">
                    @if(in_array(strtolower(pathinfo($application->id_document, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                        <img src="{{ Storage::url($application->id_document) }}" alt="ID Document" onclick="viewFullscreen('{{ Storage::url($application->id_document) }}', 'image')">
                    @elseif(strtolower(pathinfo($application->id_document, PATHINFO_EXTENSION)) === 'pdf')
                        <iframe src="{{ Storage::url($application->id_document) }}"></iframe>
                    @else
            ...
            <div class="document-card">
                <div class="document-header">
                    ...
                    <div class="document-actions">
                        <button class="doc-btn doc-btn-view" onclick="viewFullscreen('{{ Storage::url($application->proof_of_address) }}', 'image')">
                            🔍 View
                        </button>
                        <a href="{{ Storage::url($application->proof_of_address) }}" target="_blank" class="doc-btn doc-btn-download">
                            ⬇️ Download
                        </a>
                    </div>
                </div>
                <div class="file-preview">
                    @if(in_array(strtolower(pathinfo($application->proof_of_address, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                        <img src="{{ Storage::url($application->proof_of_address) }}" alt="Proof of Address" onclick="viewFullscreen('{{ Storage::url($application->proof_of_address) }}', 'image')">
                    @elseif(strtolower(pathinfo($application->proof_of_address, PATHINFO_EXTENSION)) === 'pdf')
                        <iframe src="{{ Storage::url($application->proof_of_address) }}"></iframe>
                    @else
                        <div class="file-placeholder">
                            <div class="file-placeholder-icon">📄</div>
            ...
            @if($application->company_documents)
                ...
                <div class="document-card">
                    <div class="document-header">
                        ...
                        <div class="document-actions">
                            <button class="doc-btn doc-btn-view" onclick="viewFullscreen('{{ Storage::url($doc) }}', 'image')">
                                🔍 View
                            </button>
                            <a href="{{ Storage::url($doc) }}" target="_blank" class="doc-btn doc-btn-download">
                                ⬇️ Download
                            </a>
                        </div>
                    </div>
                    <div class="file-preview">
                        @if(in_array(strtolower(pathinfo($doc, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                            <img src="{{ Storage::url($doc) }}" alt="Company Document" onclick="viewFullscreen('{{ Storage::url($doc) }}', 'image')">
                        @elseif(strtolower(pathinfo($doc, PATHINFO_EXTENSION)) === 'pdf')
                            <iframe src="{{ Storage::url($doc) }}"></iframe>
                        @else
                            <div class="file-placeholder">
                                <div class="file-placeholder-icon">📄</div>
            ...
            <div class="document-card">
                <div class="document-header">
                    ...
                    <div class="document-actions">
                        <button class="doc-btn doc-btn-view" onclick="viewFullscreen('{{ route('admin.files.view', basename($application->representative_id)) }}', 'image')">
                            🔍 View
                        </button>
                ...
                <div class="file-preview">
                    @if(in_array(pathinfo($application->representative_id, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                        <img src="{{ route('admin.files.view', basename($application->representative_id)) }}" alt="Representative ID" onclick="viewFullscreen('{{ route('admin.files.view', basename($application->representative_id)) }}', 'image')">
                    @elseif(pathinfo($application->representative_id, PATHINFO_EXTENSION) === 'pdf')
                        <iframe src="{{ route('admin.files.view', basename($application->representative_id)) }}"></iframe>
                    @else
                        <div class="file-placeholder">
                            <div class="file-placeholder-icon">📄</div>
    ...
    <div class="verification-section">
        ...
        <form method="POST" action="{{ route('admin.kyc.verify', $application->id) }}" class="verification-form">
            ...
            <div class="action-buttons">
                <button name="action" value="verify" class="action-btn btn-verify" onclick="return confirm('Are you sure you want to verify this application?')">
                    ✅ Verify Application
                </button>
                <button name="action" value="reject" class="action-btn btn-reject" onclick="return confirm('Are you sure you want to reject this application?')">
                    ❌ Reject Application
                </button>
            </div>
        </form>
